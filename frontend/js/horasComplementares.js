...
async function carregarRegras() {
    const response = await fetch(`${API_BASE_URL}${CONFIG.ENDPOINTS.regraAtividade}`, {
        method: "GET",
        headers: {
            Authorization: `Bearer ${token}`
        }
...
function atualizarCargaMinima() {
    ...
    const cursoSelecionado = cursosCarregados.find(curso =>
        Number(curso.id_curso || curso.id) === cursoIdSelecionado
    );

    inputCargaMinima.value = cursoSelecionado ? (cursoSelecionado.carga_horaria_minima || "") : "";
}
...
function limparCamposRegras() {
    document.querySelectorAll(".linha-regra").forEach((linha) => {
        ...
        const checkboxComprovante = linha.querySelector('.comprovante input[type="checkbox"]');
...
function preencherCamposRegras() {
    ...
    const regrasCurso = regrasCarregadas.filter(regra => Number(regra.curso) === cursoIdSelecionado);

    regrasCurso.forEach((regra) => {
        const tipoNome = (regra.tipo_atividade_nome || "").trim();
        const linha = document.querySelector(`.linha-regra[data-tipo="${tipoNome}"]`);
        ...
        if (checkboxComprovante) checkboxComprovante.checked = regra.exige_comprovante === true;
...
function configurarValidacaoHoras() {
    ...
    const checkboxesStatus = document.querySelectorAll('.status input[type="checkbox"]');
    const inputCargaMinima = document.getElementById("cargaMinima");
...
async function salvarRegras() {
    ...
    if (somaHoras !== cargaMinima) {
        alert(`A soma dos limites ativos (${somaHoras}h) deve ser igual à carga horária mínima do curso (${cargaMinima}h).`);
        return;
    }
    ...
    for (const linha of linhas) {
        ...
        const regraExistente = regrasCarregadas.find(regra =>
            Number(regra.curso) === cursoId &&
            normalizarTexto(regra.tipo_atividade_nome) === normalizarTexto(tipoNome)
        );

        if (regraExistente) {
            const payload = obterPayloadLinha(linha, cursoId, regraExistente.tipo_atividade);

            const response = await fetch(`${API_BASE_URL}${CONFIG.ENDPOINTS.regraAtividade}${regraExistente.id}/`, {
                method: "PATCH",
                headers: {
            ...
            const tipoBase = tiposAtividadeCarregados.find(tipo =>
                normalizarTexto(tipo.nome || tipo.tipo_atividade_nome) === normalizarTexto(tipoNome)
            );
